...
local function loadOptions()
    ...
    if not doesFileExist(file) then
        print("Copy example options to options.lua.\n")

        local success, err = copyFile(currentModDirectory .. "/options.example.lua",
            currentModDirectory .. "/options.lua")
        if not success then
            error(err)
        end
    end
...
local function loadMethod(method)
    local t = dofile(currentModDirectory .. "/Scripts/methods/" .. method .. "/main.lua")
...
local function setActorReferenceLocation()
    ...
    log.info(
        string.format(
            "ACTOR_REF_LOC { X = %.16g, Y = %.16g, Z = %.16g }",
            actorLoc.x,
            actorLoc.y,
            actorLoc.z
        )
...
local function snapActorToGrid()
    ...
    if options.set_new_location then
        hitActor:K2_SetActorLocation(
            { X = newLoc.x, Y = newLoc.y, Z = newLoc.z },
            false,
            sweepHitResult,
            false
        )
...
local function selectActorAndApplyLastOffsetAndRotation()
    ...
    ---@diagnostic disable-next-line: missing-fields
    Selected.actor:K2_AddActorLocalRotation(
        { Roll = 0, Pitch = 0, Yaw = Selected.angle },
        false,
        {},
        false
    )
...
local function resetLocalOffsetAndRotation()
    ...
    ---@diagnostic disable-next-line: missing-fields
    Selected.actor:K2_AddActorLocalOffset(
        { X = 0, Y = 0, Z = -Selected.upOffset },
        false,
        {},
        false
    )
...
local function moveActor(n_letter, n_number)
    ...
    local newLoc, args =
        snapToGrid(PARAMS.ACTOR_REF_LOC, p_actor, Selected.actor, n_letter, n_number)
    ...
    if Selected.angle ~= 0 then
        log.info("Add rotation     %.16g°", Selected.angle)
    end
    if Selected.upOffset ~= 0 then
        log.info("Add up/down offset     %.16g", Selected.upOffset)
    end
end
...
registerKeyBind(
    options.resetLocalOffsetAndRotation_Key,
    options.resetLocalOffsetAndRotation_ModifierKeys,
    resetLocalOffsetAndRotation
)

registerKeyBind(options.move.selectActor_Key, options.move.selectActor_ModifierKeys, selectActor)

registerKeyBind(
    options.move.selectActorAndApplyLast_Key,
...
registerKeyBind(
    options.move.selectedActorToRefActorLoc_Key,
...
--#region normal
registerKeyBind(
    options.move.forward_Key,
    options.move.forward_ModifierKeys,
    moveSelectedActorForward
)

registerKeyBind(
    options.move.backward_Key,
    options.move.backward_ModifierKeys,
    moveSelectedActorBackward
)

registerKeyBind(options.move.right_Key, options.move.right_ModifierKeys, moveSelectedActorRight)

registerKeyBind(options.move.left_Key, options.move.left_ModifierKeys, moveSelectedActorLeft)

registerKeyBind(options.move.up_Key, options.move.up_ModifierKeys, moveSelectedActorUp)

registerKeyBind(options.move.down_Key, options.move.down_ModifierKeys, moveSelectedActorDown)
--#endregion normal

--#region big
registerKeyBind(
    options.move.forward_big_Key,
    options.move.forward_big_ModifierKeys,
    moveSelectedActorForward_big
)

registerKeyBind(
    options.move.backward_big_Key,
    options.move.backward_big_ModifierKeys,
    moveSelectedActorBackward_big
)

registerKeyBind(
    options.move.right_big_Key,
    options.move.right_big_ModifierKeys,
    moveSelectedActorRight_big
)

registerKeyBind(
    options.move.left_big_Key,
    options.move.left_big_ModifierKeys,
    moveSelectedActorLeft_big
)

registerKeyBind(options.move.up_big_Key, options.move.up_big_ModifierKeys, moveSelectedActorUp_big)

registerKeyBind(
    options.move.down_big_Key,
...
registerKeyBind(options.snapActorToGrid_Key, options.snapActorToGrid_ModifierKeys, snapActorToGrid)

registerKeyBind(
    options.setActorReferenceLocation_Key,
    options.setActorReferenceLocation_ModifierKeys,
    setActorReferenceLocation
)

registerKeyBind(
    options.rotatePlayerToBlackLine_Key,
    options.rotatePlayerToBlackLine_ModifierKeys,
    rotatePlayerToDirection1
)

registerKeyBind(
    options.rotatePlayerToOrangeLine_Key,
...
RegisterConsoleCommandHandler("help", function(fullCommand, parameters, outputDevice)
    local helpMsg = [[# Snap to grid commands help
...
RegisterConsoleCommandHandler("help2", function(fullCommand, parameters, outputDevice)
    local helpMsg = [[# Snap to grid commands help 2
Commands:
...
RegisterConsoleCommandHandler("rloc", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: rloc <X> <Y> <Z>\n"
        .. "Example: rloc 123500 0 0\n"
        .. "Current actor reference location (ACTOR_REF_LOC): "
        .. (
            PARAMS.ACTOR_REF_LOC ~= nil
            and string.format(
                "%.16g %.16g %.16g",
                PARAMS.ACTOR_REF_LOC.x,
                PARAMS.ACTOR_REF_LOC.y,
                PARAMS.ACTOR_REF_LOC.z
            )
            or "nil"
        )
    ...
    outputDevice:Log(
        string.format(
            "New actor reference location (ACTOR_REF_LOC): %.16g %.16g %.16g",
            PARAMS.ACTOR_REF_LOC.x,
            PARAMS.ACTOR_REF_LOC.y,
            PARAMS.ACTOR_REF_LOC.z
        )
...
RegisterConsoleCommandHandler("arc", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: arc <arc length>\n"
        .. "example: arc 1000\n"
        .. "Current ARC_LENGTH: "
        .. PARAMS.ARC_LENGTH
...
RegisterConsoleCommandHandler("angle", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: angle <angle in degrees (0° to 360°)>\n"
        .. "Example: angle 90\n"
        .. "Current ANGLE: "
        .. (PARAMS.ANGLE ~= nil and PARAMS.ANGLE or "nil")
...
RegisterConsoleCommandHandler("lon_angle", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: angle <angle in degrees (-360 to 360)>\n"
        .. "Example: angle 90\n"
        .. "Current LONGITUDE_ANGLE: "
        .. (PARAMS.ANGLE ~= nil and PARAMS.ANGLE or "nil")
...
RegisterConsoleCommandHandler("rot_angle", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: rot_angle <angle in degrees (-360 to 360)>\n"
        .. "example: rot_angle 45\n"
        .. "Current ROTATION_ANGLE: "
        .. (PARAMS.ROTATION_ANGLE ~= nil and PARAMS.ROTATION_ANGLE or "nil")
...
RegisterConsoleCommandHandler("loc", function(fullCommand, parameters, outputDevice)
    ...
    local helpMsg = "Usage: loc <X> <Y> <Z>\n"
        .. "Example: loc 123500 0 0\n"
        .. "Current actor location: "
        .. string.format("%.16g %.16g %.16g", location.X, location.Y, location.Z)
    ...
    local msg = string.format(
        "New actor location: %.16g %.16g %.16g",
        newLocation.x,
        newLocation.y,
        newLocation.z
    )
...
RegisterConsoleCommandHandler("offset", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Add actor local offset\n"
        .. "Usage: offset <X> <Y> <Z>\n"
    ...
    outputDevice:Log(
        string.format(
            "Add offset (%.16g, %.16g, %.16g) on %s",
            x,
            y,
            z,
            hitActor:GetFName():ToString()
        )
    )
...
RegisterConsoleCommandHandler("rot", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Add actor local rotation\n"
    ...
    outputDevice:Log(
        string.format("Add rotation angle: %.16g on %s", angle, hitActor:GetFName():ToString())
    )
...
RegisterConsoleCommandHandler("set_new_location", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: set_new_location {true | false | 1 | 0}\n"
...
RegisterConsoleCommandHandler("set_new_rotation", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: set_new_rotation {true | false | 1 | 0}\n"
...
RegisterConsoleCommandHandler("method", function(fullCommand, parameters, outputDevice)
    local helpMsg = "Usage: method {grid | circular}\n" .. "example: method grid"
